...
var time_until_open  := 0.1 # Change if it doesn't seem to work.
...
func _enter_tree() -> void:
	...
	if Engine.get_version_info().hex >= 0x040400: # 4.4 and later.
		debugger = dumb_ref.get_parent().get_parent().get_parent().get_parent().get_child(1)
	else:
...
func _on_debugger_tab_button_pressed(tab:int) -> void:
	...
	if debugger_tabs.get_tab_button_icon(tab) == null:
		return # For some reason tabs without a texture can still emit tab_button_pressed (fixed in later versions).
